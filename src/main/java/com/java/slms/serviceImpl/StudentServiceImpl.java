...
public class StudentServiceImpl implements StudentService {
	...
	public StudentResponseDto createStudent(StudentRequestDto studentRequestDto, Long schoolId) {
		...
		Optional<Student> existingStudent = studentRepository.findByPanNumberIgnoreCaseAndSchoolIdAndSessionId(
				studentRequestDto.getPanNumber(), schoolId, session.getId());
		...
		FeeStructure feeStructure = feeStructureRepository
				.findByClassEntity_IdAndSession_IdAndSchool_Id(studentRequestDto.getClassId(), session.getId(),
						schoolId)
...
//        List<User> users = new ArrayList<>();
		...
		for (String pan : panNumbers) {
			// FIXED: Use findByPanNumberIgnoreCaseAndSchool_Id instead of
			// ...AndStatusActive
...
//        userRepository.saveAll(users);
	...
	public List<StudentResponseDto> getStudentsByClassId(Long classId, Long schoolId) {
		...
		List<Student> activeStudents = studentRepository.findByCurrentClassIdAndSchoolIdAndStatusActive(classId,
				schoolId);
	...
	public List<Student> getStudentsBySchoolIdAndPanNumbers(Long schoolId, List<String> panNumbers) {
		List<String> lowerCasePanNumbers = panNumbers.stream().map(String::toLowerCase).collect(Collectors.toList());
	...
	public StudentResponseDto updateStudent(String pan, UpdateStudentInfo updateStudentInfo, Long schoolId) {
		...
		Student fetchedStudent = studentRepository.findByPanNumberIgnoreCaseAndSchool_IdAndStatusActive(pan, schoolId)
				.orElseThrow(() -> new WrongArgumentException("Only active students can be changed. Student with PEN "
						+ pan + " is currently INACTIVE or GRADUATE"));
		...
		Long originalClassId = fetchedStudent.getCurrentClass() != null ? fetchedStudent.getCurrentClass().getId()
				: null;
		...
		if (updateStudentInfo.getClassId() != null) {
			ClassEntity classEntity = classEntityRepository
					.findByIdAndSchoolIdAndSessionActive(updateStudentInfo.getClassId(), schoolId)
			...
			Session activeSession = sessionRepository.findBySchoolIdAndActiveTrue(schoolId).orElseThrow(
					() -> new ResourceNotFoundException("No active session found for school ID: " + schoolId));
			...
			ClassEntity classEntity = classEntityRepository
					.findByClassNameIgnoreCaseAndSessionIdAndSchoolId(simpleClassName, activeSession.getId(), schoolId)
					.orElseThrow(() -> new ResourceNotFoundException(
							"Class " + simpleClassName + " not found in active session for school ID: " + schoolId));
		...
		if (classWillChange && newClassId != null && originalClassId != null) {
			log.info("Updating class_id in all related tables for student: {} from class {} to class {}", pan,
					originalClassId, newClassId);
			...
			List<Fee> feeRecords = feeRepository.findByStudentPanNumberAndSchoolIdOrderByYearAscMonthAsc(pan, schoolId,
					activeSession.getId());
		...
		if (classWillChange && newClassId != null) {
			...
			log.info("Class changed from {} to {}. Regenerating fee structure for student {}...", originalClassId,
					newClassId, pan);

			// Use repository to find fee structure
			Optional<FeeStructure> newFeeStructureOpt = feeStructureRepository
					.findByClassEntity_IdAndSession_IdAndSchool_Id(currentClass.getId(), activeSession.getId(),
							schoolId);
			...
			if (newFeeStructureOpt.isEmpty()) {
				...
				try {
					...
					List<Fee> existingFees = feeRepository.findByStudentPanNumberAndSchoolIdOrderByYearAscMonthAsc(pan,
							schoolId, activeSession.getId());
	...
	public CurrentDayAttendance getStudentsPresentToday(Optional<Long> classId, Long schoolId) {
		...
		if (classId.isPresent()) {
			Long id = classId.get();
			ClassEntity classEntity = classEntityRepository.findByIdAndSchoolId(id, schoolId)
	...
	public Student findByPanNumberIgnoreCaseAndSchool_IdAndStatusActive(String panNumber, Long schoolId) {
		return studentRepository.findByPanNumberIgnoreCaseAndSchool_IdAndStatusActive(panNumber, schoolId)
				.orElseThrow(() -> new ResourceNotFoundException("Student not found with PEN Number: " + panNumber
	...
	public void promoteStudentsToClass(List<String> panNumbers, Long classId, Long schoolId) {
		...
		List<Student> studentsToUpdate = panNumbers.stream().map(pan -> {
			Optional<Student> optionalStudent = studentRepository
					.findByPanNumberIgnoreCaseAndSchool_IdAndStatusActive(pan, schoolId);
	...
	private User fetchUserByPan(String panNumber) {
		return userRepository.findByPanNumberIgnoreCase(panNumber)
				.orElseThrow(() -> new ResourceNotFoundException("Student User not found with PEN: " + panNumber));
	}
	...
	private void validateStudentIsActive(Student student) {
		if (!student.getStatus().equals(UserStatus.ACTIVE)) {
			log.info("Update not allowed. Student with PEN {} has status: {}", student.getPanNumber(),
					student.getStatus());
			throw new WrongArgumentException("Only active students can be changed. Student with PEN "
	...
	private void setFeeStatuses(StudentResponseDto dto, String pan, Long schoolId) {
		...
		long paidMonthsFees = feeCatalog.getMonthlyFees().stream()
				.filter(fee -> "paid".equalsIgnoreCase(fee.getStatus())).count();
		...
		// Otherwise, mark as PENDING (even if overdues are cleared, but not all months
		// paid)
	...
	private PreviousSchoolingRecordDto buildSchoolingRecord(StudentEnrollments enrollment, Long schoolId) {
		...
		// We need to check if the exam's classExam's classEntity's session matches this
		// enrollment's session
		List<Score> sessionScores = allScores.stream()
				.filter(score -> score.getExam() != null && score.getExam().getClassExam() != null
		...
		// Filter scores for this session (simple approach - get all scores and
		// calculate)
		...
		Map<Long, List<Score>> scoresByExam = sessionScores.stream().filter(score -> score.getExam() != null)
		...
		for (Map.Entry<Long, List<Score>> entry : scoresByExam.entrySet()) {
			...
			if (totalMarks > 0) {
				...
				PreviousSchoolingRecordDto.ExamResultSummary examSummary = PreviousSchoolingRecordDto.ExamResultSummary
						.builder().examName(exam.getName()).examDate(null) // Exam doesn't have date in this model
						.percentage(percentage).grade(grade).obtainedMarks(obtainedMarks).totalMarks(totalMarks)
						.build();
		...
		return PreviousSchoolingRecordDto.builder().sessionId(session.getId()).sessionName(session.getName())
				.className(classLabel).section(section)
				.passingYear(session.getEndDate() != null ? session.getEndDate().getYear() : null).status(status)
				.overallPercentage(overallPercentage).overallGrade(overallGrade).totalPresent(totalPresent)
				.totalAbsent(totalAbsent).attendancePercentage(attendancePercentage).examResults(examResults).build();
	}
	...
	public void swapRollNumbers(String panNumber1, String panNumber2, Long schoolId) {
		log.info("Swapping roll numbers between students: {} and {} in school ID: {}", panNumber1, panNumber2,
				schoolId);
